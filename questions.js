...
<head>
    ...
    <title>Crisma Quest 2025</title>
    
    <!-- Bibliotecas Externas (Carregadas primeiro) -->
    <script src="https://cdn.tailwindcss.com"></script>
    <script crossorigin src="https://unpkg.com/react@18/umd/react.development.js"></script>
    <script crossorigin src="https://unpkg.com/react-dom@18/umd/react-dom.development.js"></script>
    <script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>
    <!-- Lucide é carregado aqui. O React precisa esperar que ele esteja pronto. -->
...
<body>
    <div id="root"></div> <!-- Onde o React será montado -->

    <!-- ========================================== -->
    <!-- 1. BANCO DE DADOS - LESSONS_DATA -->
    ...
    <!-- ========================================== -->
    <!-- 2. LÓGICA DO APP (Babel/JSX) -->
    <!-- Este bloco *define* os componentes, mas não os executa -->
    <!-- ========================================== -->
    <script type="text/babel">
    const { useState, useEffect } = React;
    // O objeto 'lucide' é global, acessado diretamente.

    // Helper para ícones com fallback (Definido globalmente para uso em todos os componentes)
    const Icon = ({ name, ...props }) => {
        // Mapeamento de fallback para emojis/simples HTML
        const fallback = {
            XCircle: <div className="text-lg">✖</div>,
            Heart: <div className="text-lg">❤️</div>,
            CheckCircle: <div className="text-lg">✔</div>,
            Trophy: <div className="text-lg">🏆</div>,
            Star: <div className="text-lg">⭐</div>,
            Map: <div className="text-lg">🗺️</div>,
            User: <div className="text-lg">👤</div>,
            Lock: <div className="text-lg">🔒</div>,
        };

        // 1. Tenta carregar o ícone Lucide apenas se a biblioteca lucide existir globalmente
        if (typeof lucide !== 'undefined') {
            const LucideIcon = lucide[name];
            // 2. Se o ícone for encontrado e for um componente válido (função), retorne-o
            if (typeof LucideIcon === 'function') {
                return <LucideIcon {...props} />;
            }
        }
        
        // 3. Se Lucide não estiver pronto ou o ícone não for encontrado, use o fallback de emoji/HTML
          ...
          <div className="w-full max-w-md bg-slate-800 p-8 rounded-2xl shadow-2xl border-b-4 border-slate-950">
            <div className="flex justify-center mb-6">
              <div className="bg-yellow-400 p-4 rounded-full">
                {/* Usa o componente Icon global para garantir a tipagem correta */}
                <Icon name="Trophy" size={48} className="text-yellow-900" />
              </div>
            </div>
        ...
        return (
          <div className="flex flex-col h-screen bg-white">
            <div className="p-4 border-b flex items-center justify-between">
              <button onClick={onExit}><Icon name="XCircle" className="text-slate-400 hover:text-red-500" /></button>
              <span className="font-bold text-slate-600">Introdução</span>
        ...
        return (
          <div className="flex flex-col h-screen max-w-2xl mx-auto bg-white shadow-2xl overflow-hidden">
            <div className="p-6 flex items-center gap-4">
              <button onClick={onExit}><Icon name="XCircle" className="text-slate-300 hover:text-slate-500" /></button>
              <div className="flex-1 h-4 bg-slate-200 rounded-full overflow-hidden">
                <div className="h-full bg-green-500 transition-all duration-500" style={{ width: `${progress}%` }}></div>
              </div>
              <div className="flex items-center gap-1 text-red-500 font-bold"><Icon name="Heart" fill="currentColor" /> {lives}</div>
            </div>
            ...
            <div className={`p-6 border-t ${isChecked ? (isCorrect ? 'bg-green-100' : 'bg-red-100') : 'bg-white'}`}>
              <div className="flex justify-between items-center">
                {isChecked && (
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${isCorrect ? 'bg-green-500' : 'bg-red-500'} text-white`}><Icon name={isCorrect ? "CheckCircle" : "XCircle"} size={24} /></div>
      ...
      if (step === 'completed') {
        return (
          <div className="flex flex-col h-screen items-center justify-center bg-yellow-400 p-8 text-center space-y-6">
            <Icon name="Trophy" size={80} className="text-yellow-800 animate-bounce" />
      ...
      if (step === 'failed') {
        return (
          <div className="flex flex-col h-screen items-center justify-center bg-red-500 p-8 text-center space-y-6">
            <Icon name="Heart" size={80} className="text-red-900 opacity-50" />
    ...
    const Dashboard = ({ user, progress, onSelectLesson, onLogout, lessonsData }) => {
      return (
        <div className="min-h-screen bg-slate-100 pb-20 max-w-md mx-auto shadow-2xl border-x border-slate-200">
          <div className="sticky top-0 bg-white/90 backdrop-blur-md z-10 border-b p-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-1 text-yellow-500 font-bold"><Icon name="Star" fill="currentColor" /> {progress.xp}</div>
                 <button onClick={onLogout} className="text-slate-400 hover:text-red-500"><Icon name="XCircle" size={24} /></button>
            </div>
          </div>
          <div className="p-8 flex flex-col items-center space-y-8 mt-4 relative">
            {lessonsData.map((lesson, index) => { 
              const isCompleted = progress.completedLessons.includes(lesson.id);
              const isCurrent = !isCompleted && (lesson.id === 1 || progress.completedLessons.includes(lesson.id - 1));
              
              // Determina o bloqueio: Lição 1 nunca é bloqueada. Outras são bloqueadas se a anterior não estiver completa.
              const isPreviousCompleted = lesson.id === 1 || progress.completedLessons.includes(lesson.id - 1);
              const shouldBeLocked = lesson.id !== 1 && !isPreviousCompleted;


              return (
                <div key={lesson.id} className={`relative z-10 ${index % 2 === 0 ? 'mr-12' : 'ml-12'}`}>
                  ...
                  <button disabled={shouldBeLocked} onClick={() => !shouldBeLocked && onSelectLesson(lesson)}
                    className={`w-24 h-24 rounded-full flex items-center justify-center text-4xl shadow-[0_8px_0_rgba(0,0,0,0.2)] transition-all transform active:translate-y-[8px] active:shadow-none 
                    ${isCompleted ? 'bg-yellow-400 ring-4 ring-yellow-200 text-yellow-700' : ''} 
                    ${isCurrent && !shouldBeLocked ? 'bg-blue-500 text-white ring-4 ring-blue-200' : ''} 
                    ${shouldBeLocked ? 'bg-slate-300 text-slate-400 grayscale cursor-not-allowed' : ''}`}>
                    {isCompleted ? <Icon name="CheckCircle" size={40} className="text-yellow-700" /> : (shouldBeLocked ? <Icon name="Lock" size={32} /> : lesson.icon)}
                  </button>
                  <div className="mt-3 text-center"><h3 className={`font-bold text-sm ${shouldBeLocked ? 'text-slate-400' : 'text-slate-700'}`}>{lesson.title}</h3></div>
                </div>
              );
            })}
          </div>
          <div className="fixed bottom-0 w-full max-w-md bg-white border-t p-4 flex justify-around text-slate-400">
            <button className="text-blue-500 flex flex-col items-center"><Icon name="Map" /><span className="text-xs font-bold mt-1">Aulas</span></button>
            <button className="hover:text-blue-500 flex flex-col items-center" onClick={() => console.log("Em breve: Ranking da turma!")}><Icon name="Trophy" /><span className="text-xs font-bold mt-1">Ranking</span></button>
            <button className="hover:text-blue-500 flex flex-col items-center" onClick={() => console.log("Em breve: Seu perfil completo!")}><Icon name="User" /><span className="text-xs font-bold mt-1">Perfil</span></button>
          </div>
        </div>
      );
    };
    ...
    const App = () => {
      ...
      // Carregar dados salvos e dados globais (LESSONS_DATA)
      useEffect(() => {
        // 1. Carrega dados do LocalStorage com tratamento de erros
        const savedUser = localStorage.getItem('crisma_user');
        const savedProgress = localStorage.getItem('crisma_progress');
        
        // Tenta carregar e parsear o usuário
        try {
            if (savedUser) {
            ...
            console.error("Erro ao carregar 'crisma_user' do LocalStorage. Dados corrompidos.", e);
            localStorage.removeItem('crisma_user'); // Limpa dados inválidos para evitar erro futuro
        }
        
        // Tenta carregar e parsear o progresso
        try {
            if (savedProgress) {
                const progressParsed = JSON.parse(savedProgress);
                setProgress(progressParsed);
            }
        } catch (e) {
            console.error("Erro ao carregar 'crisma_progress' do LocalStorage. Dados corrompidos.", e);
            localStorage.removeItem('crisma_progress'); // Limpa dados inválidos para evitar erro futuro
        }

        // 2. Carrega dados globais
        const globalData = window.LESSONS_DATA;
        
        if (globalData && globalData.length > 0) {
            setLessonsData(globalData);
            setIsLoading(false);
            console.log("Dados de Lições carregados com sucesso.");
        } else {
             console.error("Erro ao carregar dados: window.LESSONS_DATA está vazio ou indefinido.");
             setIsLoading(false);
        }
      }, []);
      ...
      const handleLogout = () => {
        ...
        // Opcional: Limpar LocalStorage se o logout for definitivo
        // localStorage.removeItem('crisma_user');
      };
      ...
      if (isLoading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-blue-500">
                <div className="text-white text-xl font-bold p-4 bg-blue-600 rounded-lg shadow-xl animate-pulse">
                    Carregando Dados da Crisma...
                </div>
            </div>
        );
      }
      ...
      return <Dashboard user={user} progress={progress} onSelectLesson={setActiveLesson} onLogout={handleLogout} lessonsData={lessonsData} />;
    };

    // Apenas define a função App
    </script>
    
    <!-- ========================================== -->
    <!-- 3. INICIALIZAÇÃO (Standard JS) - GARANTE EXECUÇÃO APÓS BABEL -->
    <!-- Agora usa window.onload para garantir o timing -->
    <!-- ========================================== -->
    <script>
    // Usa um script padrão para montar, garantindo que o Babel (bloco 2) já definiu <App>
    window.onload = function() {
        try {
            const rootElement = document.getElementById('root');
            // Verifica se as dependências globais foram carregadas
            if (rootElement && typeof ReactDOM !== 'undefined' && typeof App === 'function') {
                const root = ReactDOM.createRoot(rootElement);
                root.render(React.createElement(App)); // Usa React.createElement para maior compatibilidade
                console.log("Aplicação Crisma Quest montada com sucesso.");
            } else {
                 // Esta mensagem de erro é mais detalhada e ajuda a identificar problemas de carregamento de bibliotecas.
                 console.error("Erro FATAL: Dependências (ReactDOM/App) não encontradas ou #root não existe. (Verifique se Babel terminou de carregar)");
            }
        } catch (e) {
            console.error("Erro durante a montagem final do React:", e);
        }
    };
    </script>
</body>
